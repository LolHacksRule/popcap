...
bool ListWidget::MoveSelect(int theDelta)
{
	int aNewSelect = mSelectIdx + theDelta;
	if (aNewSelect < 0)
		aNewSelect = 0;
	else if (aNewSelect >= (int) mLines.size())
		aNewSelect = mLines.size() - 1;

	if (aNewSelect == mSelectIdx)
		return false;

	SetSelect(mSelectIdx);
	mSelectIdx = aNewSelect;
	MarkDirty();
	return true;
}

bool ListWidget::KeyDown(KeyCode theKey)
{
	...
	{
		if ((mSelectIdx != -1) && (mListListener != NULL))
			mListListener->ListClicked(mId, mSelectIdx, 1);
		return true;
	}
	else
	{

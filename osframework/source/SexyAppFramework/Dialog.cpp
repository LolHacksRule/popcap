...
void Dialog::RemovedFromManager(WidgetManager* theWidgetManager)
{
	...
	{
		if (mYesButton != NULL)
			RemoveWidget(mYesButton);
		if (mNoButton != NULL)
			RemoveWidget(mNoButton);
	}
}
...
void Dialog::Resize(int theX, int theY, int theWidth, int theHeight)
{
	...
	if ((mYesButton != NULL) && (mNoButton != NULL))
	{
		int aBtnWidth = (mWidth - mContentInsets.mLeft - mContentInsets.mRight - mBackgroundInsets.mLeft - \
				 mBackgroundInsets.mRight - mButtonSidePadding*2 - mButtonHorzSpacing) / 2;
		int aBtnHeight = mButtonHeight;

		mYesButton->Resize(x + mBackgroundInsets.mLeft + mContentInsets.mLeft + mButtonSidePadding,
				   y + mHeight - mContentInsets.mBottom - mBackgroundInsets.mBottom - aBtnHeight,
	...
	else if (mYesButton != NULL)
	{
		...
		mYesButton->Resize(x + mContentInsets.mLeft + mBackgroundInsets.mLeft,
				   y + mHeight - mContentInsets.mBottom - mBackgroundInsets.mBottom - aBtnHeight,
				   mWidth - mContentInsets.mLeft - mContentInsets.mRight - mBackgroundInsets.mLeft - \
				   mBackgroundInsets.mRight, aBtnHeight);
	}
}

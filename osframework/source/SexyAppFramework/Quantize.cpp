...
bool Sexy::Quantize8Bit(const uint32* theSrcBits, int theWidth, int theHeight, uchar* theDestColorIndices, uint32* theDestColorTable)
{
	...
	for (int anIdx = 1; anIdx < aSize; anIdx++)
	{
		...
		{	
			...
			if (aLeftPos > aRightPos)
			{
				...
				memmove(aSearchTable+anInsertPos+1, aSearchTable+anInsertPos, (aColorTableSize-anInsertPos) * sizeof(uint32));
